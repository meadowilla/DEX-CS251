...
const token_name = 'MEADOWILLA';                // TODO: replace with name of your token
const token_symbol = 'MDWL';              // TODO: replace with symbol for your token
...
const exchange_contract = new ethers.Contract(exchange_address, exchange_abi, defaultAccount);
...
async function init() {
    ...
    if (poolState['token_liquidity'] === 0
      ...
      await exchange_contract.connect(provider.getSigner(defaultAccount)).createPool(5000, { value: ethers.utils.parseUnits("5000")});
...
function calculateMaxMinExchangeRate(maxSlippagePct){
  maxExchangeRate = exchange_rate_multiplier + ((maxSlippagePct * exchange_rate_multiplier)/100);
  minExchangeRate = exchange_rate_multiplier - ((maxSlippagePct * exchange_rate_multiplier)/100);
  return {maxExchangeRate, minExchangeRate};
}
...
async function swapTokensForETH(amountToken, maxSlippagePct) {
  ...
  console.log(maxExchangeRate);
  console.log(amountToken);
  console.log(Number(amountToken));
  await exchange_contract.connect(provider.getSigner(defaultAccount)).swapTokensForETH(Number(amountToken), maxExchangeRate);
}
